...
const manifestPath = path.join(root, 'batch7-18-downloaded-indexnow-urls.json');
const sourcePath = path.join(root, 'batch7-18-downloaded-source-slugs.json');
const ctaUrl = '/?signup=true&role=client';
const factDate = '2026-06-13';
const batchNumbers = Array.from({ length: 12 }, (_, index) => index + 7);
...
function factNote(slug, type) {
  if (/ay-2026-27|fy-2025-26/.test(slug)) {
    return 'For AY 2026-27, the return generally relates to FY 2025-26 under the existing return utility. Do not mix it with Tax Year 2026-27 concepts unless the official utility or law clearly applies.';
  }
  if (/income-tax-act-2025|it-act-2025|it-rules-2026|tax-year|april-2026|2026-27/.test(slug)) {
    return 'Income Tax Act, 2025 and Income-tax Rules, 2026 topics are date-sensitive. Verify commencement, transition provisions, rule numbers and portal utilities from official sources before relying on summaries.';
  }
...
function intent(slug) {
  if (/near-me|best-|top-|find-|affordable/.test(slug)) return 'Compare professionals';
  if (/deadline|due-date|calendar|penalty|late-fee/.test(slug)) return 'Deadline and risk planning';
  if (/documents|required|checklist/.test(slug)) return 'Document checklist';
  if (/registration|incorporation|apply|process|step-by-step/.test(slug)) return 'Registration process';
  if (/return|filing|form-|gstr|itr|tds|tcs/.test(slug)) return 'Return filing';
  if (/notice|appeal|assessment|scrutiny|litigation|demand/.test(slug)) return 'Notice and dispute support';
  if (/rate|rates|calculator|limit|threshold|slab/.test(slug)) return 'Rate and threshold check';
  if (/audit|valuation|due-diligence|review/.test(slug)) return 'Review and assurance';
...
function topicSpecificBullets(slug, type, title) {
  ...
  if (type === 'finance') {
    return [
      `Confirm whether the topic is tax, RBI/FEMA, SEBI, bank, valuation or investment reporting before relying on a rate or threshold.`,
...
function updateSitemap() {
  ...
  sitemapPath && fs.writeFileSync(sitemapPath, `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.join('\n')}\n</urlset>\n`, 'utf8');
...
function main() {
  ...
  for (const page of pages) {
    ...
    if (fs.existsSync(out) && process.env.OVERWRITE_BATCH7_18 !== '1') {
