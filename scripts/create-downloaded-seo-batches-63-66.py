...
manifest_path = root / "batches63-66-downloaded-indexnow-urls.json"
source_path = root / "batches63-66-downloaded-source-slugs.json"
progress_file = Path("C:/Ravish/indexer/progress.json")
urls_file = Path("C:/Ravish/indexer/urls.txt")

cta_url = "/?signup=true&role=client"
fact_date = "2026-06-27"
batch_numbers = [63, 64, 65, 66]
    ...
    if category == "gift_city_ifsc":
        ...
        return [
            ...
            ("How is municipal tax paid on house property treated for tax?",
             "Municipal taxes paid by the owner during the financial year are directly deductible from the Gross Value (GAV) of the property to compute the Net Annual Value (NAV)."),
        ...
        return [
            ...
            ("How are legacy angel tax assessments defended?",
             "Startups must defend valuations using Rule 11UA (DCF/NAV reports) and utilize DPIIT recognition certificates to claim exemptions."),
            ...
            ("Does Section 56(2)(x) apply to share issuances?",
             "Yes. While Section 56(2)(viib) (company-side tax) is abolished, Section 56(2)(x) (tax on recipient of shares for inadequate consideration) remains fully active."),
        ...
        return [
            ...
            ("How is GST applied on legal and accounting services?",
             "Reverse charge GST applies to legal services by advocates. Accounting services are charged 18% GST by the provider."),
            ("What are the core reporting points in CARO 2020?",
             "Auditors report on inventory verification, fixed asset titles, loan defaults, internal audit systems, and statutory dues compliance."),
            ("Is Profession Tax mandatory for businesses in India?",
             "Yes, Profession Tax is state-specific. PT registration and monthly deductions (typically ₹200/month) are mandatory."),
            ("What is Section 44AD presumptive taxation?",
             "Eligible businesses with turnover under ₹2 crore (₹3 crore if digital) can declare 6% or 8% of turnover as taxable income."),
            ("How are customs duties determined on audits?",
             "Customs duty is calculated based on classification, transaction value, basic customs duty, social welfare surcharge, and IGST."),
            ("What is an AD Code registration and why is it needed?",
             "Authorized Dealer (AD) Code registration with port customs is mandatory for exporters to generate shipping bills and claim benefits."),
...
def main():
    ...
    # 2. Extract pages from batch 63 to 66
    raw_pages = []
