...
manifest_path = root / "batches50-53-downloaded-indexnow-urls.json"
source_path = root / "batches50-53-downloaded-source-slugs.json"
cta_url = "/?signup=true&role=client"
fact_date = "2026-06-22"
batch_numbers = [50, 51, 52, 53]
...
def fact_note(slug, type_name):
    # Senior citizen / super senior citizen tax
    if re.search(r'senior-citizen|super-senior-citizen|pensioner|section-80ttb|form-15h|retirement|gratuity|pension', slug):
        return 'For senior citizens (aged 60-79) and super senior citizens (aged 80+), the basic exemption limit under the Old Tax Regime is ₹3,00,000 and ₹5,00,000 respectively. Under the default New Tax Regime (FY 2025-26), the basic exemption is ₹4,00,000 for all individuals regardless of age. Senior citizens can claim up to ₹50,000 deduction on interest income under Section 80TTB, and are exempt from paying advance tax under Section 207 if they do not have business income.'
    # RSUs / ESOPs / Foreign share taxation
    if re.search(r'rsu|esop|stock-option|foreign-share|vesting|exercise|perquisite|section-17|schedule-fa|foreign-employer', slug):
        return 'RSUs and ESOPs are taxed twice: first as a salary perquisite under Section 17 on the exercise/vesting date (based on Fair Market Value minus exercise price), and second as capital gains when sold (LTCG at 12.5% if held >12 months for listed or >24 months for unlisted/foreign shares). For resident Indians, foreign shares (like US stocks/RSUs) must be disclosed in Schedule FA of the ITR, failing which a penalty of ₹10 lakh under the Black Money Act may apply.'
    # Agricultural income taxation
    if re.search(r'agricultural-income|agriculture|farm-income|section-2-1a|section-10-1|partial-integration|agricultural-land', slug):
        return 'Agricultural income is exempt from tax under Section 10(1) of the Income Tax Act. However, if your agricultural income exceeds ₹5,000 and your other taxable income exceeds the basic exemption limit, the integration method (partial integration) is used to compute the tax: agricultural income is added to non-agricultural income to determine the slab rate for the non-agricultural portion.'
    # Secretarial audit
    if re.search(r'secretarial-audit|section-204|form-mr-3|company-secretary|secretarial-standard|mca-compliance', slug):
        return 'Under Section 204 of the Companies Act, 2013, secretarial audit is mandatory for listed companies and unlisted public companies with paid-up share capital >= ₹50 crore or turnover >= ₹250 crore. The secretarial audit report must be in Form MR-3 and signed by a practicing Company Secretary (CS).'
