...
manifest_path = root / "batches73-76-downloaded-indexnow-urls.json"
source_path = root / "batches73-76-downloaded-source-slugs.json"
...
def main():
    ...
    # Subtract slugs present in the current batch files so they can be regenerated
    current_batch_slugs = set()
    for num in batch_numbers:
        for p in extract_pages(f"workindex-seo-batch{num}.md", num):
            current_batch_slugs.add(p['slug'].lower())
